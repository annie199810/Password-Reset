...
if (
  ...
  console.warn(
    'WARNING: Running in production without a secure JWT_SECRET. Set JWT_SECRET in environment!'
  );
}
  ...
  try {
    ...
    return res.json({
      ...
      message: mailResult
        ...
        : 'Reset token created (email delivery failed).',
      previewUrl:
...
router.post('/reset-password', async (req, res) => {
  ...
  if (!token || !emailNorm || !password)
    return res
      .status(400)
      .json({ error: 'token, email and password required' });
